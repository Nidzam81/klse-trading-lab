"""
IOI Corporation (1961.KL) KLSE Trading Agent v1.0
====================================================
Strategy: RSI(14) <= 30 + SMA200 trend filter on 30-min candles
Entry: RSI <= 30 AND price > SMA200
Exit: 2.0% TP / 1.0% SL
Account: FUTUMY (acc_id=286260079099239522)
Backtest: 3 trades, 66.7% WR, PF 5.2, RM21 PnL
"""
...
TICKER = "MY.1961"; QTY = 100
RSI_PERIOD = 14; RSI_BUY = 30; SMA_PERIOD = 200
TP_PCT = 0.02; SL_PCT = 0.01; KLINE_BARS = 250
LOG_DIR = os.path.join(os.path.expanduser("~"), "AppData", "Local", "hermes", "ioi_agent")
...
def upload_trade(s,p,q,r,ep=0,pnl=0):
    try:
        ...
        upload_trade_log_for_agent("ioi",s,p,q,r,ep,pnl)
    except: pass
def upload_state(price=None,rsi14=None,sma200=None,trend=None,reason="periodic"):
    try:
        sys.path.insert(0,os.path.dirname(os.path.abspath(__file__)))
        ...
        upload_state_for_agent("ioi",st,latest_entry={"position_open":st.get("position_open"),"price":price,"rsi14":rsi14,"sma200":sma200,"trend":trend,"reason":reason,"entry_price":ep,"qty":st.get("qty"),"tp":tp,"sl":sl})
    except: pass
def load_state():
...
def calc_sma(v,p): return round(sum(v[-p:])/p,4) if len(v)>=p else None
def fetch_data():
    ...
    r=p["data"]
    if len(r)<SMA_PERIOD: return None,None,None
    c=[x["close"] for x in r if"close" in x and x["close"] is not None]
    if not c: return None,None,None
    return c[-1],calc_sma(c,SMA_PERIOD),calc_rsi(c,RSI_PERIOD)
def place_order(s,q):
...
def main():
    ...
    if pos and ep>0:
        ...
        if price is None: log_event({"type":"exit_check","status":"no_price"})
            ...
            log_event({"type":"exit_signal","reason":"TP","entry":ep,"current":price,"msg":f"IOI TP! {ep:.2f}->{price:.2f} (+{(price/ep-1)*100:.2f}%)"})
    ...
    cp,sma200,rsi=fetch_data()
    if cp is None or sma200 is None or rsi is None: log_event({"type":"data_fail","msg":"Failed to fetch IOI data"}); return
    log_event({"type":"market_data","price":cp,"sma200":sma200,"rsi14":rsi,"msg":f"IOI: price={cp:.2f} SMA200={sma200:.2f} RSI={rsi}"})
    if cp<=sma200:
        log_event({"type":"trend_filter","passed":False,"msg":f"Bearish: {cp:.2f} <= SMA200={sma200:.2f}"})
        upload_state(price=cp,rsi14=rsi,sma200=sma200,trend="BEAR",reason="scan"); return
    if rsi<=RSI_BUY:
        log_event({"type":"buy_signal","rsi":rsi,"price":cp,"msg":f"IOI BUY! RSI={rsi}<={RSI_BUY} price={cp:.2f}"})
        ...
        if ok:
            ...
            log_event({"type":"order_filled","side":"BUY","price":cp,"qty":QTY,"order_id":res,"msg":f"BUY {QTY} IOI @ {cp:.2f} order={res} TP={tp:.2f} SL={sl:.2f}"})
            send_telegram(f"IOI BUY SIGNAL\nBUY {QTY} @ RM{cp:.2f}\nRSI: {rsi} SMA200: RM{sma200:.2f}\nTP: RM{tp:.2f} SL: RM{sl:.2f}\nOrder: {res}")
            upload_trade("BUY",cp,QTY,"ENTRY",0,0)
        else: log_event({"type":"order_failed","side":"BUY","error":res})
    else: log_event({"type":"no_signal","rsi":rsi,"msg":f"RSI not oversold ({rsi}>{RSI_BUY})"})
    upload_state(price=cp,rsi14=rsi,sma200=sma200,trend="BULL" if cp>sma200 else "BEAR",reason="scan")
